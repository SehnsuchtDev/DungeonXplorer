...
CREATE TABLE `MCQTest` (
                           `ce_id` INT PRIMARY KEY NOT NULL,
                           `mcqt_question` varchar(255) NOT NULL,
                           `mcqta_anwser_num` INT NOT NULL
);

CREATE TABLE `MCQAnswer` (
                             `mcqt_answer_num` INT,
                             `ce_id` INT NOT NULL,
                             `mcqt_answer` varchar(255) NOT NULL
);
...
CREATE UNIQUE INDEX `MCQAnswer_index_0` ON `MCQAnswer` (`mcqt_answer_num`, `ce_id`);
...
ALTER TABLE `MCQAnswer` ADD FOREIGN KEY (`ce_id`) REFERENCES `MCQTest` (`ce_id`);

ALTER TABLE `Monster` ADD FOREIGN KEY (`ce_id`) REFERENCES `ChapterEvent` (`ce_id`);

ALTER TABLE `MCQTest` ADD FOREIGN KEY (`ce_id`) REFERENCES `ChapterEvent` (`ce_id`);
