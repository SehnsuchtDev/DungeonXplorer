...
<div id="chapDiv" class="pagediv bg-[#F6F0E8] h-full w-full filter drop-shadow-lg shadow-inner">
    ...
    <div class="p-6 font-['Pirata_One'] justify-items-center text-center">
        ...
        <?php if ($mcq): ?>
            ...
            <br>
            <form id="mcqTest<?= $seed ?>" class="text-left" method="post"
                action="<?= constant('FULLURLROOTPATH') ?>/book/page/chapter/mcqtest">
                <?php foreach ($mcqChoices as $key => $choice): ?>
                    <input class="pointer-events-auto" type="radio" name="choice" value="<?= $key ?>"><?= $choice ?></input><br>
                <?php endforeach; ?>
                <br>
                <input class="pointer-events-auto bg-[#e4b87f] p-1 rounded cursor-pointer" type="submit" value="Valider">
            </form>
        <?php elseif ($fight): ?>
            ...
            <div class="bg-[#C4975E] py-3 px-6 m-3 text-2xl rounded max-w-xs mx-auto pointer-events-auto cursor-pointer">
                <a class="pageButton<?= $seed ?>" href="<?= constant('FULLURLROOTPATH') ?>/book/page/chapter/fight"><button
                        <?= $eventIsDone ? 'disabled' : '' ?>><?= $fightStatus ?></button></a>
            </div>
        <?php endif; ?>
        <br>
